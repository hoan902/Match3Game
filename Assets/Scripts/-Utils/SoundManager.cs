...
public class SoundManager : MonoBehaviour
{
    ...
    //HoanDN Can be use after implement save function (using PlayerPref)
    ...
    public static GameObject PlaySound(AudioClip clip, bool loop, bool music = false, bool unpause = false, float volume = 1f)
    {
        if (music)
        {
            if (s_api.m_music != null)
        ...
        else
        {
            ...
            source.Play(clip, loop);
            AudioSource audioComp = go.GetComponent<AudioSource>();
            audioComp.volume = s_api.m_soundOn ? volume : 0;
